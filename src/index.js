...
const fetch_releases = (releases_url = RELEASES_URL) =>
...
const resolve_url_tpl = (releases, platform_key, ver) => {
    ...
    if (!platform?.url_tpl) return null;
...
const resolve_sdk = async (platform_key, version = 'latest') => {
    ...
    if (!platform_data) {
        ...
        throw new Error(
            `Unknown platform '${platform_key}'. ` + `Available: ${available}`,
        );
    }
    const ver = version == 'latest' ? platform_data.last_version : version;
    if (!ver)
        throw new Error(`No latest version for platform '${platform_key}'`);
    const url =
        platform_data.url || resolve_url_tpl(releases, platform_key, ver);
    if (!url) {
        throw new Error(
    ...
    if (platform_data.sha256) result.sha256 = platform_data.sha256;
    return result;
};
...
const extract = async (archive_path, out_dir) => {
    if (archive_path.endsWith('.tar.gz') || archive_path.endsWith('.tgz'))
        execFileSync('tar', ['-xzf', archive_path, '-C', out_dir]);
...
const fetch_sdk = async (platform_key, version = 'latest', output = '.') => {
    ...
    const ext = url_path.endsWith('.tar.gz')
        ? '.tar.gz'
...
const list_platforms = async () => {
    const releases = await fetch_releases();
    return Object.entries(releases.platforms || {}).map(([key, val]) => ({
        key,
