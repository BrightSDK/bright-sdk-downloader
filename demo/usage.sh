...
echo "               |___/                             "
printf '\e[0m'
echo "  bright-sdk-download — CLI Demo"
echo
sleep 1
...
_prompt "bright-sdk"
_out "bright-sdk — BrightSDK download CLI
...
_prompt "bright-sdk resolve -p android"
...
_prompt "bright-sdk resolve -p ios -v 1.620.3"
_json '{"platform":"ios","version":"1.620.3","url":"https://cdn.example.com/sdk/ios/1.620.3/brd_sdk_ios.tar.gz"}'
sleep 1

# Step 5: Fetch (download + extract)
_banner "--- Fetch SDK → download + extract ---"
_prompt "bright-sdk fetch -p tizen -o ./libs"
_out "Fetching tizen@latest..."
sleep 0.8
_out "Done → ./libs"
...
# Step 6: Fetch for Gradle/CI usage
_banner "--- Use in CI pipeline (exit code) ---"
_prompt "bright-sdk fetch -p android -o ./app/libs && echo 'OK'"
_out "Fetching android@latest...
Done → ./app/libs"
